      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '16px', marginBottom: '30px' }}>
        ...
        {users.slice(0, 3).map((user, index) => {
          ...
          const shadows = [
            ...
            '0 4px 15px rgba(205, 127, 50, 0.5)'
          ];
          ...
          return (
            <div key={index} style={{
              ...
              <div style={{ fontSize: '2.5rem', marginBottom: '5px' }}>{medalEmoji[index]}</div>
              <h3 style={{ fontSize: '1.2rem', fontWeight: 'bold', margin: '5px 0' }}>{titles[index]}</h3>
              <h2 style={{ fontWeight: 'bold', fontSize: '1.5rem', marginBottom: '10px' }}>{maskName(user.username)}</h2>
              <p style={{ marginBottom: '5px' }}>
                Wager: <strong>${user.total?.toLocaleString(undefined, { minimumFractionDigits: 3 })}</strong>
              </p>
              <p>
                Payout: <strong>${payout.toFixed(3)}</strong>
              </p>
            </div>
          );
        })}
